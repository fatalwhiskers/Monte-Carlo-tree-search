...
/*!
This class contains the UCT program.
*/
	...
	*/
	move runTree(tState rootstate, int iterations)
	{
		...
		for (int i = 0; i < iterations; i++)
		{
			...
			// selection process
			while (nod->fullyExpanded() && nod->hasChild()) //fully expanded and not a leaf
			{
				//select child
				nod = nod->selectBestChildWithUCT();
				//commit the move
